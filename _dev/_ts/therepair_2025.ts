...
import ModalBox from './components/modules/ModalBoxMovieImgNone';
namespace Main {
	export class Page extends Base.PageCore {
		...
		private mv() {
			...
			// TweenMax.fromTo($('.mv-title__main'), 1, { alpha: 0, marginTop: 20 }, { alpha: 1, marginTop: 0, delay: 0.5});
			// TweenMax.fromTo($('.mv-title__lead'), 1, { alpha: 0, marginTop: 20 }, { alpha: 1, marginTop: 0, delay: 1});
			TweenMax.fromTo($('.mv-title__text'), 1, { alpha: 0 }, { alpha: 1 ,delay: 0.5});
			TweenMax.fromTo($('.mv-title__logo'), 1, { alpha: 0}, { alpha: 1, delay:1 });
		}
